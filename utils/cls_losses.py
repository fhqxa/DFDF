...
class LADELoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        num_samples_per_cls = torch.sum(target == torch.arange(0, self.num_classes).view(-1, 1).type_as(target),
                                        -1).float()  # C
...
def build_cls_loss(cfg, cls_num_list, device):
    cls_num_list = torch.tensor(cls_num_list, device=device)

    loss_type = cfg.LOSS.CLS.NAME
    ...
    if loss_type == "CE":
        ...
        return LDAMLoss(cls_num_list=cls_num_list, s=cfg.LOSS.CLS.SCALE)

    elif loss_type == "CB":
        return ClassBalancedLoss(cls_num_list=cls_num_list)

    elif loss_type == "GRW":
        return GeneralizedReweightLoss(cls_num_list=cls_num_list)

    elif loss_type == "BS":
        return BalancedSoftmaxLoss(cls_num_list=cls_num_list)

    elif loss_type == "LA":
        return LogitAdjustedLoss(cls_num_list=cls_num_list)

    elif loss_type == "LADE":
        return LADELoss(cls_num_list=cls_num_list)
